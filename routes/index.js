...
router.get('/', function(req, res) {
  // admins.create({Email: "ritesh@gmail.com", Password: "12345"}, function(err, obj) {
  //   if(err)
  //   {
  //     console.log(err);
  //   }
    ...
    if(err){
      ...
      bcrypt.compare(Admin_Login.Password, obj.Password, function(err, result) {
        ...
        else if(result==false)
        {
          ...
          var token = jwt.sign({data: obj._id}, "blkhrt", {expiresIn: "1h"});
          res.cookie("token", token, {maxAge: 3600000, httpOnly: true}).send([true]);
          console.log(result);
        }
      });
    }
